...
def run(img_path, visualize=False):
    labels = load_labels('mscoco_label_map.pbtxt')
    ...
    for img, img_name in image_generator:
        object_detect_net.setInput(cv2.dnn.blobFromImage(img, size=(640, 640), swapRB=True, crop=False))
        ...
        for detection in object_detect_output[0, 0]:
            ...
            if score > 0.30:
                #Pridam aj kolko plochy dany objekt zabera?
